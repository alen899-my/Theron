<template>
  <div class="rounded-xl border border-border bg-card shadow-sm overflow-hidden">
    ...
    <div
      ...
      class="border-t border-red-500/20 bg-red-500/8 px-4 sm:px-5 py-3 flex items-start gap-2.5"
    >
      <span class="text-red-500 shrink-0 mt-0.5 text-sm leading-none">✕</span>
      <div>
        <div class="text-xs font-semibold text-red-400 mb-0.5">Job failed</div>
        <div class="text-xs font-mono text-red-400/80">{{ job.errorMessage }}</div>
      </div>
    </div>
  </div>
</template>
